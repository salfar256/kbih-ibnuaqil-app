...
export default function LoginGate({ logo }) {
  ...
  const kirim = async () => {
    ...
    try {
      ...
      else { await lupaSandi(email); setInfo("Tautan atur ulang kata sandi sudah dikirim ke email Anda."); }
    } catch (e) { setGalat(pesanGalat(e)); }
    setSibuk(false);
  };

  const judul = mode === "masuk" ? "Masuk Pembimbing" : mode === "daftar" ? "Daftar Akun Pembimbing" : "Lupa Kata Sandi";
  const inputStyle = { width: "100%", padding: "12px 12px 12px 40px", border: `1px solid ${C.border}`, borderRadius: 12, background: "#fff", fontSize: 14, fontFamily: "inherit", color: C.ink };

  return (
    <div style={{ fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", minHeight: "100vh", background: `linear-gradient(150deg, ${C.greenDeep} 0%, ${C.green} 55%, #12693e 100%)`, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, position: "relative", overflow: "hidden" }}>
      <style>{`
      ...
      <div style={{ background: "#fff", borderRadius: 24, padding: 30, width: 400, maxWidth: "100%", boxShadow: "0 24px 60px rgba(0,0,0,.28)", position: "relative" }}>
        ...
        <p style={{ margin: "0 0 18px", fontSize: 12.5, color: C.muted }}>
          {mode === "masuk" ? "Masuk untuk mengakses data jamaah." : mode === "daftar" ? "Buat akun untuk pembimbing baru." : "Masukkan email terdaftar Anda."}
        </p>

        <div style={{ position: "relative", marginBottom: 12 }}>
          <Mail size={17} color={C.muted} style={{ position: "absolute", left: 13, top: 14 }} />
        ...
        {mode !== "lupa" && (
          <div style={{ position: "relative", marginBottom: 12 }}>
            <KeyRound size={17} color={C.muted} style={{ position: "absolute", left: 13, top: 14 }} />
        ...
        {info && (
          <div style={{ background: C.greenSoft, color: C.green, borderRadius: 10, padding: "10px 12px", fontSize: 12.5, marginBottom: 12 }}>{info}</div>
        )}
        ...
        <div style={{ marginTop: 16, textAlign: "center", fontSize: 12.5, color: C.muted, display: "flex", flexDirection: "column", gap: 8 }}>
          {mode === "masuk" && (
            <>
              <button className="lg-btn" onClick={() => { setMode("lupa"); setGalat(""); setInfo(""); }} style={{ background: "none", color: C.muted, fontWeight: 500, fontSize: 12.5 }}>Lupa kata sandi?</button>
              <div>Pembimbing baru? <button className="lg-btn" onClick={() => { setMode("daftar"); setGalat(""); setInfo(""); }} style={{ background: "none", color: C.green, fontSize: 12.5, padding: 0 }}>Daftar di sini</button></div>
            </>
          )}
          {mode !== "masuk" && (
